...
const generateSeoMeta = (post: Post, twitterOptions: TwitterOptions) => {
  ...
  return {
    ...
    seoMeta: {
      ...
      ogSiteName: post.seo?.opengraphSiteName ?? 'All Halacha',
      ogImageWidth: post.seo?.opengraphImage?.mediaDetails?.width ?? 700,
      ogImageHeight: post.seo?.opengraphImage?.mediaDetails?.height ?? 400,
